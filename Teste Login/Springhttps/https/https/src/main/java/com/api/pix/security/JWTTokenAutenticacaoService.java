...
import com.api.pix.ApplicationContextLoad;
...
public class JWTTokenAutenticacaoService {
	...
	public void addAuthentication(HttpServletResponse response, String username) throws Exception {
		...
		String empresa= "remarca";
		/*Dá a resposta pra tela e para o cliente, outra API, navegador, aplicativo, javascript, outra chamadajava*/
		response.addHeader(HEADER_STRING, token);
		response.addHeader(empresa, "remarca");
		liberacaoCors(response);
         String empresa1= "remarca1";
		/*Usado para ver no Postman para teste*/
		response.getWriter().write("{\"Authorization\": \"" + token + "\"}");
		//response.getWriter().write("{\"EMPRESA\": \"" + empresa1 + "\"}");

	}

	public void addAuthenticationFaild(HttpServletResponse response) throws IOException {

		/*Usado para ver no Postman para teste*/
		response.getWriter().write("{\"Authorization\": \"Erro\"}");
